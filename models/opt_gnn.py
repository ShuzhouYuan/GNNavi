...
class OPTDecoderwithGNN(OPTDecoder):
    ...
    def forward(
        ...
        for idx, decoder_layer in enumerate(self.layers):
            ...
            if self.n_layer != 1:
                if idx == self.middle_layer_index:
                ...
                elif idx == self.last_layer_index:
                    orig_size = hidden_states.size()
...
class OPTModelwithGNN(OPTModel):
    def __init__(self, config: OPTConfig):
        super().__init__(config)
        self.decoder = OPTDecoderwithGNN(config)
...
class OPTForCausalLMwithGNN(OPTForCausalLM):
    _tied_weights_keys = ["lm_head.weight"]

    def __init__(self, config):
        super().__init__(config)
        self.model = OPTModelwithGNN(config)
