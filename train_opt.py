...
from models.opt_gnn import OPTForCausalLMwithGNN
from transformers import OPTConfig, OPTForCausalLM, AutoTokenizer, get_linear_schedule_with_warmup
...
class ICLforClassification(LightningModule):
    def __init__(
        ...
        config = OPTConfig.from_pretrained(model_name_or_path, num_hidden_layers=n_layer)

        if self.exp_type == 'gnn':
            self.model = OPTForCausalLMwithGNN.from_pretrained(self.model_name_or_path, config=config)
            for name, param in self.model.named_parameters():
                if "gnn" not in name:
                    param.requires_grad = False
        elif self.exp_type == 'vanilla':
            self.model = OPTForCausalLM.from_pretrained(self.model_name_or_path, config=config)
        elif self.exp_type == 'vanilla-peft':
            model = OPTForCausalLM.from_pretrained(self.model_name_or_path, config=config)
    ...
    def add_model_specific_args(parser):
        ...
        parser.add_argument("--model_name", type=str, default='facebook/opt-1.3b', help="name of the model")
...
def main(args):
    ...
    icldata = ICLDataModule(args.task_name, args.num_test, args.num_demo_per_class, args.batch_size, tokenizer, model_max_length=2048)
